...
if(!class_exists('WC_Shipping_Method'))
	return;
class PR_Pardakht_Sefareshi_COD_Method extends WC_Shipping_Method
{
	var $w_unit = '';

	public function __construct()
	{
		$this->id = 'pr_pardakht_sefareshi_cod';
		$this->method_title = 'پست سفارشی - پرداخت در محل';
	...
	public function init_form_fields()
	{
		$this->form_fields = array(
			'enabled' => array(
				...
				'label'     => 'فعال کردن پست سفارشی - پرداخت در محل',
				'type'      => 'checkbox',
				'default'   => 'no',
			),
			'title' => array(
				'title'     => 'عنوان روش',
				'type'      => 'text',
				'default'   => 'پست سفارشی - پرداخت در محل',
			),
			'min_amount' => array(
	...
	public function calculate_shipping( $package )
	{
		...
		if ( sizeof( $woocommerce->cart->get_cart() ) > 0 && ( $customer->get_shipping_city() ) )
		{
			...
			$data['weight'] = $weight;
			$data['deliveryID'] = '2'; // sefareshi
			$data['paymentID'] = '1'; // cod
	...
	function pardakht_shipping( $data = false )
	{
		...
		if( pr_pardakht_is_free_send($woocommerce) === true )
			return 0;
